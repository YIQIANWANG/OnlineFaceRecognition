...
(function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        showRefresh: false,
        showToggleQuery: false, // TODO 扩展属性，显示/异常查询条件
        showToggle: false,
    ...
    // TODO 自定义修改数据加载遮罩层class='fixed-table-loading' , 新增fixed-table-loading-msg, 同时修改$tableLoading关联的全部地方
    // 修改后的遮罩层要结合外部framework.js来定位显示
    BootstrapTable.prototype.initContainer = function () {
        this.$container = $([
            '<div class="bootstrap-table">',
            '<div class="fixed-table-loading">',
            '<div class="fixed-table-loading-msg"><i class="fa fa-refresh fa-spin"></i> '+this.options.formatLoadingMessage()+'</div>',
            '</div>',
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        // TODO 扩展属性，显示/异常查询条件
        if (this.options.showToggleQuery) {
        	html.push(sprintf('<button class="btn' +
        			sprintf(' btn-%s', this.options.buttonsClass) +
        			sprintf(' btn-%s', this.options.iconSize) +
        			'" type="button" name="toggleQuery" aria-label="toggleQuery" title="%s">', "显示/隐藏查询"),
        			sprintf('<i class="%s %s"></i>', "fa", "fa-filter"),
        	'</button>');
        }
        ...
        if (this.options.search) {
            ...
            // TODO 改进：搜索框后边添加查询按钮
            html.push(
            		sprintf('<div class="%s-%s search btn-group">', bs.pullClass, this.options.searchAlign),
            		sprintf('<button class="btn-query btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + '" type="button" name="query" aria-label="query" title="搜索">'),
            		sprintf('<i class="glyphicon glyphicon-search"></i>'),'</button>',
            		sprintf('<button class="btn-reset btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + '" type="button" name="reset" aria-label="reset" title="清空">'),
            		sprintf('<i class="glyphicon glyphicon-erase"></i>'),'</button>',
            	'</div>');
            // END
            ...
            // TODO 改进：搜索框后边添加查询按钮-添加事件
            var $searchBtn = this.$toolbar.find('.btn-query');
            $searchBtn.off('click').on('click', function (event) {
            	var text = $.trim($search.val());

                // trim search input
                if (that.options.trimOnSearch && $search.val() !== text) {
                    $(event.currentTarget).val(text);
                }

//                if (text === that.searchText) {
//                    return;
//                }
            ...
            var $resetBtn = this.$toolbar.find('.btn-reset');
            $resetBtn.off('click').on('click', function (event) {
            	var text = "";
            	$search.val(text);
                that.searchText = text;
                that.options.searchText = text;
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        // TODO 自定义无数据展示背景图片
        var noRecord__ = this.$body.find(".no-records-found");
        var fixTableBody__ = this.$body.parents(".fixed-table-body");
        if(noRecord__.length > 0 ) {
        	fixTableBody__.addClass("no-records-bg");
        	noRecord__.hide();
        } else {
        	fixTableBody__.removeClass("no-records-bg");
        }
    };
    ...
    BootstrapTable.prototype.initServer = function (silent, query, url) {
        ...
        if (this.options.ajax) {
            ...
            // TODO 请求数据中加入当前显示的列的属性字段
            var columns_ = this.options.columns[0];
            var fields = "";
            for(var i = 0, len = columns_.length; i < len; i++){
            	var c = columns_[i];
            	if( !c.checkbox && !c.radio && c.field && c.switchable != false) {
            		fields += c.field + ",";
            	}
            }
            if(fields != "") fields = fields.substring(0, fields.length-1);
            request.data.fields = fields;
            //END
    ...
    BootstrapTable.prototype.resetView = function (params) {
        ...
        if (this.options.height) {
            ...
            this.$tableContainer.css('height', height + 'px');  // TODO : 标记： 计算表格内容的高度部分
        }
