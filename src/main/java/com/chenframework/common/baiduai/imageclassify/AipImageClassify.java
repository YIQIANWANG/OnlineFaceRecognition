...
package com.chenframework.common.baiduai.imageclassify;
...
public class AipImageClassify extends BaseClient {
    ...
    /**
     * 通用物体识别接口 该请求用于通用物体及场景识别，即对于输入的一张图片（可正常解码，且长宽比适宜），输出图片中的多个物体及场景标签。
     * @param image - 二进制图像数据
    ...
    /**
     * 菜品识别接口 该请求用于菜品识别。即对于输入的一张图片（可正常解码，且长宽比适宜），输出图片的菜品名称、卡路里信息、置信度。
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表: top_num 返回预测得分top结果数，默认为5
     * @return JSONObject
     */
    ...
    /**
     * 车辆识别接口 该请求用于检测一张车辆图片的具体车型。即对于输入的一张图片（可正常解码，且长宽比适宜），输出图片的车辆品牌及型号。
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表: top_num 返回预测得分top结果数，默认为5
     * @return JSONObject
     */
     ...
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表: custom_lib 是否只使用自定义logo库的结果，默认false：返回自定义库+默认库的识别结果
     * @return JSONObject
     */
    ...
    /**
     * 动物识别接口 该请求用于识别一张图片。即对于输入的一张图片（可正常解码，且长宽比适宜），输出动物识别结果
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表: top_num 返回预测得分top结果数，默认为6
     * @return JSONObject
     */
    ...
    /**
     * 植物识别接口 该请求用于识别一张图片。即对于输入的一张图片（可正常解码，且长宽比适宜），输出植物识别结果。
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表:
     * @return JSONObject
     */
    ...
    /**
     * 图像主体检测接口 用户向服务请求检测图像中的主体位置。
     * @param image - 二进制图像数据
     * @param options - 可选参数对象，key: value都为string类型 options - options列表: with_face
